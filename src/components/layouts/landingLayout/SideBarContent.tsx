...
const SideBarContent = ({ setOpen }: SideBarContentProps) => {
  ...
  return (
    <>
      <h1 className="text-xl font-mono font-bold  flex gap-4 ">

            <Link href="/">
            <span className=' text-fuchsia-600 px-0.5  bg-white'>M</span>
              oussaif
      ...
      <div className="flex flex-col gap-5 text-lg">
        {sections.map((s, i) => (
          <AnimatedWrapper delay={0.15 * i} key={s.id} from="left">
            <Link
      ...
      <div className="flex flex-col gap-7.5">
          ...
          <div className="flex gap-3 p-1.5">
            {socialLinks.map((s) => (
              <Link
                ...
                target="_blank"
                href={s.href}
                className="p-2 bg-background dark:bg-background/10 rounded-full
                           transition-all duration-300 ease-in-out
                           hover:scale-110 
                           hover:bg-linear-to-r  hover:from-indigo-300  hover:to-pink-300"
              >
                <Image
        ...
        <AnimatedWrapper from="left" delay={0.30}>
          <p>
            Copyright &copy;2025 Abdessamade Moussaif. All rights reserved.
          </p>
        </AnimatedWrapper>
      </div>
    </>
